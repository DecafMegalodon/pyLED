...
class LedStrip:
    ...
    def __init__(self, port_name, led_count, baud=115200):
        try:
            __serial_con = serial.Serial(port=port_name, baudrate=baud, timeout=1)
            if __serial_con.is_open:
                byte2 = led_count % 256
                byte1 = (led_count - byte2) >> 8
                barry = bytearray()
                barry.append(byte1)
                barry.append(byte2)
                __serial_con.write(barry)
                if int(__serial_con.readline()) == led_count:
...
test = LedStrip(port_name =  "/dev/ttyACM0", led_count = 110)

# arduino = init_connection("/dev/ttyACM0", 115200, 110)
# send_command(arduino, 2, 0, 255, 0, 0)
